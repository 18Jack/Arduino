...
void setup() {
  ...
  nextServoMove = rtc.now() + TimeSpan(0, 0, time, 0); // Configurar el próximo movimiento en 1 minuto
...
void initservo(){
  ...
  if (nowtime >= nextServoMove && !isMoving) {
    isMoving = true;
    moveEndTime = nowtime + TimeSpan(0, 0, time, 0); // Configurar el fin del movimiento en 1 minuto
  }
  ...
  if (isMoving) {
    ...
    if (nowtime >= moveEndTime) {
      isMoving = false;
      nextServoMove = nowtime + TimeSpan(0, 0, time, 0); // Configurar el próximo movimiento en 1 minuto
    }
  }
}
...
void readEncoder() {
  ...
  if (dtValue == HIGH) {
    ...
    else if (selecttime) time = max(time - 1, 0); // Limitar time a un mínimo de 0
  }
  newContent = true;
}
